var app = (function() {
    ...
    let filterByDate = document.querySelector(".date-input");
    let filterBtnDate = document.querySelector(".btn-sort-date");
    let filterBy
    filterBtnDate.addEventListener("click", filterDateClick);

    function filterDateClick() {
        filterBy = filterByDate.value;
        let albumsFiltered
        if (filterBy === "") {
            return renderAlbums(allAlbums);
        } else {
            
            albumsFiltered = allAlbums.filter(function(album) {
