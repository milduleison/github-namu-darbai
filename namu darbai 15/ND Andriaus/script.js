...
function addAlbum() {
    ...
    if (!artist || !album || !release || !image) {
        console.log("ivesta neteisingai" + " "); //jei neivedei atlikejo
        return;
    }
...
function isAlbumYear() {
    ...
    // Patikrinam ar skaicius
    let isNumber = isFinite(data);
    if (!isNumber) {
        alert("Įvestas ne skaičius");
        return;
    }
        
    // Patikrinam ar papuolą į gerą metų rėžį
    if (data > 1900 && data < 2018) {
